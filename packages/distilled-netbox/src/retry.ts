/**
 * NetBox retry surface — a veneer over `@distilled.cloud/core/retry`.
 *
 ...
 * NetBox calls below it and core's `makeDefault` is the fallback when none
 * is provided.
 *
 * @example
 * ```ts
 ...
 *
 * myEffect.pipe(Netbox.Retry.transient);
 * ```
 */
...
/** Context tag for configuring retry behavior of NetBox API calls. */
export class Retry extends Context.Service<Retry, Policy>()("NetboxRetry") {}

/** Provides a custom retry policy to every NetBox API call below it. */
export const policy: {
  (
    options: Options,
  ): <A, E, R>(
