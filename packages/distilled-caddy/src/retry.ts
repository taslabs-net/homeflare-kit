/**
 * Caddy retry surface — a veneer over `@distilled.cloud/core/retry`.
 *
 ...
 * Caddy calls below it and core's `makeDefault` is the fallback when none is
 * provided.
 *
 * @example
 * ```ts
 ...
 *
 * myEffect.pipe(Caddy.Retry.transient);
 * ```
 */
...
/** Context tag for configuring retry behavior of Caddy admin API calls. */
export class Retry extends Context.Service<Retry, Policy>()("CaddyRetry") {}

/** Provides a custom retry policy to every Caddy API call below it. */
export const policy: {
  (
    options: Options,
  ): <A, E, R>(
