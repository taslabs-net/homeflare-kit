...
| --- | --- | --- |
| Headlamp | Skip Distilled / Alchemy vendor family | Install via Argo Application / Helm. `/config` and the cluster proxy are not a vendor control plane. |
| Traefik | No Distilled SDK | No official OpenAPI — do not invent one. Declarative config is Kubernetes CRDs / Helm values (experimental flags as Helm props). Prefer `@distilled.cloud/kubernetes` and upstream Alchemy Kubernetes resources (S1). Dashboard `GET /api/…` is read-only runtime — optional later, not the dogfood install path. |
| Cilium | No hand-written interim | Install, BGP and policy via Helm + CRDs (`@distilled.cloud/kubernetes` / Alchemy Kubernetes). Official swagger exists (`cilium/cilium` `api/v1/openapi.yaml`) but the default transport is the unix socket `/var/run/cilium/cilium.sock`. Generate+copy later only if node-local agent health is needed. |
| CNPG | Kubernetes Distilled | `@distilled.cloud/kubernetes` CRDs (`postgresql.cnpg.io/v1`). |
| Valkey | Kubernetes Distilled | Operator CRDs through `@distilled.cloud/kubernetes`. Sentinel is RESP, not HTTP. |
| Argo CD | `@distilled.cloud/argocd` | Real npm package. [PR 240](https://github.com/taslabs-net/homeflare-kit/pull/240) stays draft until a Talos-on-PVE cluster exists. |
| Talos CLI | leave alone | `@homeflare/alchemy/talos` stays `talosctl`. No Distilled Talos API in this path. |
...
Addresses use RFC 5737 documentation ranges. Secret *values* never appear —
