...
3. **Observe the path.** A symlink or directory there is refused. A file this
   ...
   `--adopt` the new name takes the file, and the old name's delete leaves a
   path another declaration installed in the same deploy (`adopt-parity.test.ts`).
...
Every refusal, by where it is refused (the stack program, the plan, the apply,
the vendor's bytes, the read-back): [release-binary-refusals.md](./release-binary-refusals.md).
