...
Seventeen existing PVE resource families use named distilled operations throughout:
ACL, ApiToken, BackupJob, CephDaemon, CephFs, CephOsd, CephPool, Group, MetricServer,
NodeNetwork, NotificationMatcher, NotificationTarget, Pool, Role, Storage, User and
...
`homeflare-proxmox` at commit `91b31b0`. Its opt-in vault-door Lxc is still a later
slice. This source migration alone does not establish live adoption.
...
Twelve existing families still retain some hand-client transport: Lxc, Vm (QEMU),
...
- Lxc task polling must keep the credential that started the task, preserve bounded
  uncertainty after connection loss, and retain its cluster-wide identity checks.
