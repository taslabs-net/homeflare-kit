 ...
 *   The distilled PBS SDK owns that distinct header; `distilled-pbs.ts` supplies its credentials.
 *
 * ⚠️ HISTORICAL BOOTSTRAP BLOCKER, 2026-09-13: THERE WAS NO PBS MOUNT. `PbsTarget.mount` is
 *   a parameter for the reason `PveTarget.mount` is — the provider must be able to leave this
 *   estate. A later PBS credential lane resolved the following incident. MEASURED: under the agent's approle,
 ...
 *   ⚠️ PBS HAD NO DYNAMIC-SECRETS PLUGIN OF ITS OWN — OpenBao's Proxmox support is a PVE thing — so
...
import type { PbsDatastoreAttributes, PbsTarget } from './pbs-datastore-form.ts';
import { handlers } from './pbs-datastore-lifecycle.ts';
...
export const PbsDatastoreProvider = () =>
  Provider.effect(PbsDatastore, Effect.succeed(PbsDatastore.Provider.of(handlers)));
