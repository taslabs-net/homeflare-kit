 ...
 *   not; the install below names it, as the README does. `/forgejo` (2026-09-23, moved off a
 *   hand-rolled `HttpClient` client) imports `@distilled.cloud/forgejo` the same way, but it is
 *   NOT alchemy's own dependency — nothing hides a missing peer for it either way, and the
 *   install below still names it, matching the README.
