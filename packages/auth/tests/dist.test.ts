...
describe.skipIf(!built)('dist/index.js', () => {
  test('exports a real createD1AuthStorage, not a tree-shaken name', async () => {
    const mod = await import(distUrl.href);

    expect(typeof mod.createD1AuthStorage).toBe('function');
    expect(typeof mod.VERSION).toBe('string');
    expect(mod).not.toHaveProperty('betterAuth');
    expect(mod).not.toHaveProperty('createAuth');
  });
