/**
 * Paperless-ngx retry surface — a veneer over `@distilled.cloud/core/retry`.
 *
 ...
 * Paperless-ngx calls below it and core's `makeDefault` is the fallback when
 * none is provided.
 *
 * @example
 * ```ts
 ...
 *
 * myEffect.pipe(PaperlessNgx.Retry.transient);
 * ```
 */
...
/** Context tag for configuring retry behavior of Paperless-ngx API calls. */
export class Retry extends Context.Service<Retry, Policy>()(
  "PaperlessNgxRetry",
) {}

/** Provides a custom retry policy to every Paperless-ngx API call below it. */
export const policy: {
  (
    options: Options,
  ): <A, E, R>(
