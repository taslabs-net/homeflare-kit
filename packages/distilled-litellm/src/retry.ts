/**
 * LiteLLM retry surface — a veneer over `@distilled.cloud/core/retry`.
 *
 ...
 * LiteLLM calls below it and core's `makeDefault` is the fallback when none
 * is provided.
 *
 * @example
 * ```ts
 ...
 *
 * myEffect.pipe(Litellm.Retry.transient);
 * ```
 */
...
/** Context tag for configuring retry behavior of LiteLLM API calls. */
export class Retry extends Context.Service<Retry, Policy>()("LitellmRetry") {}

/** Provides a custom retry policy to every LiteLLM API call below it. */
export const policy: {
  (
    options: Options,
  ): <A, E, R>(
