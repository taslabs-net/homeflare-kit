...
- Where Alchemy skips that probe (props holding an Output, as on `caddyWithFile()`'s first deploy), the apply refuses the same takeover before any `/load`. It resolves adoption as the planner does: the resource's own `adopt(…)`, else `--adopt`. So `.pipe(adopt(false))` still refuses under `--adopt`, and `.pipe(adopt(true))` takes over without it. The HostFile is written by then.
- The state vouches only for the Caddy it was applied to. When the transport now reaches a Caddy at another endpoint, the apply needs `--adopt` unless that Caddy runs the config the state last stored, the declared one, or nothing.
