...
One foreground Sonnet adversarial review completed before opening this PR, aimed at
restart-safety and secret leakage. Both findings were real and are fixed:

- **Restart-safety (high).** `reconcileContainer`'s reload gate (`wrote ||
preStatus.needDaemonReload`) could not see that the SOURCE `.container` file had already been
  ...
  rendered) too, the same signal `changed` already used to gate the restart. New regression test:
  container-generator.test.ts, "an interrupted apply that crashed before daemon-reload".
- **Secret leakage (high).** `containerProblems` only ran `secretLikeEnvironment` over
  ...
  `containerProblems`. New tests in container-secrets.test.ts cover each field.
