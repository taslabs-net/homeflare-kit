...
- **Each privileged argv is logged before it runs.** The log holds the argv only, never file
  content.
- **These are refused before sudo is asked:** any argv outside the allowlist (such as a bare
  `bootout system` or a directory `bootstrap`), a path outside every prefix that needs root, a
  symlink between the prefix and the file, a file the operator could not read back, and another
